...
export function HealthScoreGauge({ score, band, size = 160 }: Props) {
  ...
  return (
    ...
    >
      {/* Donut + score only. The score is short (≤3 chars), so it always fits the
          hole; the band label lives below the chart so it can never overlap the
          ring at any size, zoom, or breakpoint. */}
      <div
        className="relative"
        style={{ width: size, height: size }}
        data-testid="gauge-viz"
      >
      ...
      <span className="max-w-full text-center text-2xs uppercase tracking-wide text-text-tertiary">
        {BAND_LABEL[band]}
      </span>
    </div>
  );
