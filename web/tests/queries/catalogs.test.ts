...
describe("catalog hooks", () => {
  ...
  it("creates a catalog and it appears in the workspace listing", async () => {
    ...
    expect(list.result.current.data!.some((c) => c.slug === "staging")).toBe(
      true,
    );
  });
  ...
  it("attaches an existing catalog to a second workspace (M:N)", async () => {
    const { wrapper } = createWrapper();
    const attach = renderHook(() => useAttachCatalog("acme-research"), {
      wrapper,
    });
    let attached;
    await act(async () => {
      attached = await attach.result.current.mutateAsync({
        catalog: "curated",
      });
    });
    // The same catalog now reports two bindings.
    expect(
      (attached as { attached_workspaces: number }).attached_workspaces,
    ).toBe(2);
  });

  it("detaches a catalog from a workspace", async () => {
    const { wrapper } = createWrapper();
    const detach = renderHook(() => useDetachCatalog("acme-analytics"), {
      wrapper,
    });
    await act(async () => {
    ...
    await waitFor(() => expect(list.result.current.isSuccess).toBe(true));
    expect(list.result.current.data!.some((c) => c.slug === "curated")).toBe(
      false,
    );
  });
});
