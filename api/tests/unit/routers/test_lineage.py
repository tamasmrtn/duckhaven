...
async def test_backfill_is_queued_not_run_in_the_request(auth_client, ws):
    """The request only records the intent. A workspace with months of history
    must not be able to hold a request open, or compete with the queries it is
    reading about."""
    resp = await auth_client.post(f"/workspaces/{ws}/lineage/backfill", json={})

    assert resp.status_code == 202
    body = resp.json()
    assert body["status"] == "pending"
    assert body["queries_scanned"] == 0
    assert body["dry_run"] is False


async def test_backfill_accepts_a_bound_and_a_dry_run(auth_client, ws):
    resp = await auth_client.post(
        f"/workspaces/{ws}/lineage/backfill",
        json={"since": "2026-01-01T00:00:00Z", "dry_run": True},
    )
...
async def test_backfill_status_is_readable(auth_client, ws):
    await auth_client.post(f"/workspaces/{ws}/lineage/backfill", json={})
    resp = await auth_client.get(f"/workspaces/{ws}/lineage/backfill")
...
async def test_backfill_status_is_404_before_one_is_requested(auth_client, ws):
    assert (await auth_client.get(f"/workspaces/{ws}/lineage/backfill")).status_code == 404


async def test_a_second_backfill_while_one_is_outstanding_is_refused(auth_client, ws):
    await auth_client.post(f"/workspaces/{ws}/lineage/backfill", json={})
    resp = await auth_client.post(f"/workspaces/{ws}/lineage/backfill", json={})
    assert resp.status_code == 409


async def test_backfill_can_be_cancelled(auth_client, ws):
    await auth_client.post(f"/workspaces/{ws}/lineage/backfill", json={})
    resp = await auth_client.delete(f"/workspaces/{ws}/lineage/backfill")

    assert resp.status_code == 200
    # Cooperative: the runner stops at the next batch boundary, so the status
    # here is still the one it was queued with.
    assert resp.json()["status"] == "pending"


async def test_backfill_requires_owner(client, ws, db_session, owner):
    """It reads every statement the workspace has ever run and writes lineage
    across every catalog it attaches — the same reach as retiring a provider,
    which is already owner-only."""
    writer = User(email="w@l.local", password_hash=hash_password("pw"), name="W", role="user")
    ...
    await client.post("/auth/login", json={"email": "w@l.local", "password": "pw"})

    assert (await client.post(f"/workspaces/{ws}/lineage/backfill", json={})).status_code == 403
    assert (await client.get(f"/workspaces/{ws}/lineage/backfill")).status_code == 403
    assert (await client.delete(f"/workspaces/{ws}/lineage/backfill")).status_code == 403


# --- graph response shape ---------------------------------------------------


async def test_the_graph_reports_completeness_and_per_provider_freshness(auth_client, ws):
    await auth_client.post(
        f"/workspaces/{ws}/lineage/imports",
        json={"provider": "custom", "edges": [_edge()]},
    )
    resp = await auth_client.get(
        f"/workspaces/{ws}/catalogs/warehouse/schemas/analytics/tables/dim/lineage"
    )
