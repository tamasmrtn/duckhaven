...
async def test_dispatch_injects_trace_context(db_session, span_exporter):
    ...
    # A null origin (a user's query) reports as "interactive".
    assert span.attributes["duckhaven.origin"] == "interactive"


async def test_dispatch_span_labels_scheduled_origin(db_session, span_exporter):
    user = User(email="sched@test.local", password_hash=hash_password("pw"), name="S", role="user")
    db_session.add(user)
    await db_session.flush()
    ...
    query = Query(
        workspace_id=ws.id, agent_id=agent.id, user_id=user.id, sql="SELECT 1", origin="scheduled"
    )
    db_session.add(query)
    await db_session.flush()
    ...
    spans = span_exporter.get_finished_spans()
    span = next(s for s in spans if s.name == "dispatch_query")
    assert span.attributes["duckhaven.origin"] == "scheduled"
