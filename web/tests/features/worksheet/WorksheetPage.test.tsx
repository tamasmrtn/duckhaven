...
describe("WorksheetPage run", () => {
  ...
  it("surfaces a rejected dispatch as a readable error", async () => {
    // A disallowed query returns a 422 carrying the guard's own machine code;
    // the worksheet must show the envelope's `message`.
    server.use(
  ...
  it("opens the assistant with the failed query prefilled, without sending it", async () => {
    ...
    await user.click(
      within(panel).getByRole("button", { name: "Close assistant" }),
    );
    await waitFor(() =>
      expect(
