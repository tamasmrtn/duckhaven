...
describe("UsersPage", () => {
  ...
  it("grants a workspace role from the manage-workspaces dialog", async () => {
    ...
    await user.click(
      await screen.findByRole("button", { name: /actions for jess/i }),
    );
    await user.click(
      await screen.findByRole("menuitem", { name: /manage workspaces/i }),
    );

    // The dialog lists the user's workspaces; grant a role in "finance" (no access).
    const select = await screen.findByRole("combobox", {
      name: /role in finance/i,
    });
    await user.click(select);
    await user.click(await screen.findByRole("option", { name: "writer" }));

    await waitFor(() =>
      expect(put).toEqual({ url: "finance", role: "writer" }),
    );
  });
