...
| `SESSION_MAX_BUCKET_FRACTION` | No | `1.0` | Ceiling on how much of the agent's budget one session statement may grow to. `1.0` lets a single heavy statement use the whole agent when nothing else is running; lower it to hold more back for other tenants. |
