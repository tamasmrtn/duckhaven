...
async def _get_service_account_or_404(db: AsyncSession, sa_id: uuid.UUID) -> User:
    user = await db.get(User, sa_id)
...
@router.patch("/{sa_id}", response_model=ServiceAccountOut)
async def update_service_account(
    ...
    account = await _get_service_account_or_404(db, sa_id)
...
async def delete_service_account(
    ...
    query_count = await db.execute(
        select(func.count()).select_from(Query).where(Query.user_id == sa_id)
    )
    ...
    await db.execute(delete(WorkspaceMember).where(WorkspaceMember.user_id == sa_id))
...
@router.post("/{sa_id}/pat", response_model=PatTokenOut, status_code=status.HTTP_201_CREATED)
async def issue_pat(
    sa_id: uuid.UUID,
...
@router.get("/{sa_id}/pats", response_model=list[PatOut])
async def list_pats(
    sa_id: uuid.UUID,
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_permission(Permission.SERVICE_ACCOUNTS_MANAGE)),
    ...
    result = await db.execute(
        select(Credential)
        .where(Credential.user_id == sa_id, Credential.kind == "pat")
        .order_by(Credential.created_at)
    )
    return list(result.scalars().all())


@router.delete("/{sa_id}/pat/{pat_id}", status_code=status.HTTP_204_NO_CONTENT)
